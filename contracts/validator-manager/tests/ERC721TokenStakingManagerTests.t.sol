...
contract ERC721TokenStakingManagerTest is PoSValidatorManagerTest, IERC721Receiver {
    ...
    ERC721Manager public erc721Manager;
    ...
    function testDisableInitialization() public {
        ...
        app.initialize(_defaultPoSSettings(), erc721Manager);
    }
    ...
    function _defaulteERC721Settings() internal pure returns (ERC721ManagerSettings memory) {
        return ERC721ManagerSettings({
            _token: IERC721(address(0)),
            _posManager: ERC721TokenStakingManager(address(0)),
            minimumNFTAmount: 1,
            maximumNFTAmount: 50,
            balanceTrackerNFT: TrackingRewardStreams(address(0))
        });
    }
    ...
    function _setUp() internal override returns (IValidatorManager) {
        ...
        erc721Manager = new ERC721Manager(ICMInitializable.Allowed);

        stakingToken.setApprovalForAll(address(erc721Manager), true);

        PoSValidatorManagerSettings memory defaultPoSSettings = _defaultPoSSettings();
        ERC721ManagerSettings memory defaultERC721Settings = _defaulteERC721Settings();
        ...
        defaultERC721Settings._token = stakingToken;
        defaultERC721Settings._posManager = app;
        defaultERC721Settings.balanceTrackerNFT = balanceTrackerNFT;
        ...
        balanceTrackerNFT.registerReward(address(erc721Manager), address(rewardToken), 0, amounts);
        balanceTrackerNFT.enableReward(address(erc721Manager), address(rewardToken));
        ...
        erc721Manager.initialize(defaultERC721Settings);
        app.initialize(defaultPoSSettings, erc721Manager);
